...
void AvgPulseShape(){
  ...
  for(int i = 0; i<nentries; i++){
	...
	if((QDCch2>135000)&&(QDCch2<160000)){
	  ...
	  for(int j = 1; j<2; j++){//loop over the PMTs
  ...
  Double_t xVals[2520];
  Double_t AvgLEDONch1[2520];
  Double_t AvgBeamONch1[2520]; 
  for(int i=0; i<2520; i++){
    xVals[i] = i;
    AvgLEDONch1[i] = avgLEDONSignal.at(1).at(i);
    AvgBeamONch1[i] = avgBeamONSignal.at(1).at(i);
  }
  ...
  for(int i=0; i<100; i++){
    BeamDivLED[i] = AvgBeamONch1[i+(354-50)]/AvgLEDONch1[i+(1033-50)];
    xVals2[i] = i;
    ScaleLEDON[i] = AvgLEDONch1[i+(1033-50)]*(100./18.5);
    ScaleBeamON[i] = (AvgBeamONch1[i+(354-50)])*(100./670.);
    //printf("ScaleBeamON[%i] = %f...    ", i, ScaleBeamON[i]);
    //printf("AvgBeamONch1[%i+(354-50)] = %f...   ", i, AvgBeamONch1[i+(354-50)]);
  }

//const TVectorD& xValsConst = xVals;
// const TVectorD& AvgLEDONch1 = avgLEDONSignal.at(1);
//const TVectorD& AvgBeamONch1 = avgBeamONSignal.at(1);

  TCanvas* c3 = new TCanvas("LED ON avg", "Average LED ON waveform", 4);
  TGraph* LEDONgraph = new TGraph(2520, xVals, AvgLEDONch1);
  LEDONgraph->GetXaxis()->SetTitle("Time (ns)");
  LEDONgraph->Draw("AL");

  TCanvas* c4 = new TCanvas("Beam ON avg", "Average Beam ON waveform", 5);
  TGraph* BeamONgraph = new TGraph(2520, xVals, AvgBeamONch1);
  BeamONgraph->GetXaxis()->SetTitle("Time (ns)");
  BeamONgraph->Draw("AL");

  TCanvas* c5 = new TCanvas("Beam divided by LED",
  ...
  TCanvas* c6 = new TCanvas("Test Canvas",
